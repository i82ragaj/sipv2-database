...
-- Return: Nothing
-- Summary:	Insert and update all parking counters
-- =============================================
CREATE   PROCEDURE [AA].[InsertAllParkingCounters]
@frecuency varchar(1) = 'N'
AS
BEGIN
	...
	SELECT [ID], [Name], [type]
	FROM [AA-ADMIN].[dbo].[MDParking] where [Active] = 1 and [Frecuency] = @frecuency;
	...
	BEGIN
		BEGIN TRY
		if(@type = 'SD')
			set @sql ='EXEC [SD].[InsertParkingCounter] ''' + @DBNAME + '''';
